---
title: Resources
menu:
  header:
    weight: 4
---

Things I found useful and fun for computational sociologists (mostly open source). 

## Software

Paperpile: A browser-based reference manager that downloads PDFs and saves notes and annotations to Google Drive
[[website](https://paperpile.com/features/)]

Obsidian: A powerful knowledge base on top of a local folder of plain text Markdown files
[[github](https://obsidian.md/)]

Grobid: A machine learning library for extracting information from scholarly PDFs
[[github](https://github.com/kermitt2/grobid)]

## Data

Data is Plural: A weekly newsletter of useful/curious datasets by Jeremy Singer-Vine
[[website](https://www.data-is-plural.com/)]

Awesome Public Datasets: A topic-centric list of high quality open datasets
[[github](https://github.com/awesomedata/awesome-public-datasets)]

## Methods

MIT 18.06: Linear Algebra by Gilbert Strang
[[website](http://41.89.4.180/courses/mathematics/18-06sc-linear-algebra-fall-2011/)]

Yale MGMT 737: Applied Empirical Methods for Management PhDs by Paul Goldsmith-Pinkham
[[github](https://github.com/paulgp/applied-methods-phd)]

Text Mining with R by Julia Silge and David Robinson 
[[website](https://www.tidytextmining.com/)]

## Visualization

From Data to Viz: leads you to the most appropriate graph for your data, links to the code to build it, and lists common caveats you should avoid, by Yan Holtz and Conor Healy 
[[website](https://www.data-to-viz.com/)]

MetBrewer: Color palette package in R and Python inspired by artworks at the Met by Blake Robert Mills
[[github](https://github.com/BlakeRMills/MetBrewer)]

## Presentation

Demystifying the Dissertation Proposal by Leonard Cassuto
[[website](https://www.chronicle.com/article/demystifying-the-dissertation-proposal/)]

Writing Tips for PhD Students by John H. Cochrane
[[PDF](https://static1.squarespace.com/static/5e6033a4ea02d801f37e15bb/t/5eea8ee7c4488718b640f3c6/1592430312374/phd_paper_writing.pdf)]

The Introduction Formula by Keith Head
[[website](http://blogs.ubc.ca/khead/research/research-advice/formula)]

Slides on making slides by Kieran Healy
[[website](https://kieranhealy.org/blog/archives/2018/03/24/making-slides/)]

Writing Proposals for ACLS Fellowship Competitions by Christina M. Gillis
[[PDF](https://www.acls.org/wp-content/uploads/2021/10/Writing-Fellowship-Proposals-1.pdf)]

An Essay on the Art of Reviewing by Myra Marx Ferree
[[PDF](https://www.ssc.wisc.edu/gender/wp-content/uploads/2014/04/Myras-Tipsheet0804.pdf)]